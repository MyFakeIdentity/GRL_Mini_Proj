...
import torch
from ultra.utils_data_gen import compute_counts
...
def build_relation_graph(graph):
    ...
    print("Starting homomorphism counting")
    hom_counts = compute_counts(edge_index.tolist())

    hom_count_array = torch.zeros((num_rels, len(hom_counts)), device=device)
    for feature in range(len(hom_counts)):
        for vertex in hom_counts[feature]:
            hom_count_array[vertex, feature] = hom_counts[feature][vertex]

    # print(hom_count_array.shape, num_rels, num_nodes)

    rel_graph = Data(
        pattern_data=hom_count_array,
        edge_index=edge_index, 
        edge_type=torch.cat([hh_edges[:, 2], tt_edges[:, 2], ht_edges[:, 2], th_edges[:, 2]], dim=0),
        num_nodes=num_rels, 
        num_relations=4,
        num_additional_features=len(hom_counts),
    )
